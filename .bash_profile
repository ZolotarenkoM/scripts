...
# Git
gcm () { git add . && git commit -m "$1" && git push origin HEAD; }
...
function valjen() {
  local jenkinsfile=$1
	curl -u <user>:<token> -X POST -F "jenkinsfile=<${jenkinsfile}" \
          https://your-jenkins.com/pipeline-model-converter/validate
}
